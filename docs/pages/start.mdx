...
class Algorithm {
     ...
     */
    encode() {
        this.bindings.output.value = yourEncodeMethod(this.bindings.input) // Access the input value from the bindings
    }
    /**
     ...
     */
    decode() {
        this.bindings.input.value = yourDecodeMethod(this.bindings.input)
    }
}
```
