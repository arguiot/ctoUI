...
# Creating components
Components represent the basic building block in CTO UI. They are fundamental in making it easy to build complex UIs from little building blocks. They are also responsible for managing their internal status and communicating (via the dispatcher) to other components when it changes.
